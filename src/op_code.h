...
enum class OpCode {
  ...
  ALLOCL,       // allocate list obj, push oid x
  ADDLI,        // pops x, adds new list element to x
  SETLE,        // pop x, pop y, set obj(x)[-1] = y
  SETLI,        // pop z, pop y, pop x, set obj(x)[y] = z 
  GETLI,        // [operand] pop x, push value of obj(x)[v]
  LNUMI,        // pop x, push num of ints in obj(x)
  LNUMD,        // pop x, push num of doubles in obj(x)        
  LNUMS,        // pop x, push num of strings in obj(x)
  LNUMB,        // pop x, push num of bools in obj(x)
  LRMB,         // pop x, remove last element of obj(x)
  LAVGI,        // pop x, push avg of ints in obj(x)
  LAVGD,        // pop x, push avg of doubles in obj(x)
  LSIZE,        // pop x, push size of obj(x)
  LRETRIEVE,    // pop x, pop y, push obj(y)[x]
